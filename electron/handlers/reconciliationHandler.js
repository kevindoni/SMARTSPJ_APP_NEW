...
function getOpeningBalance(db, year, month) {
  ...
  if (month === 1) {
    ...
    return {
      ...
      tunai: 0,
      total: 0,
      details: { lainnya: { bank: 0, tunai: 0 }, reguler: { bank: 0, tunai: 0 } },
    };
  }
}
...
function getExpenses(db, year, month) {
  ...
  let lainnyaBJ = 0,
    lainnyaModalMesin = 0,
    lainnyaModalAset = 0;
  let lainnyaBJ_Tunai = 0,
    lainnyaModalMesin_Tunai = 0,
    lainnyaModalAset_Tunai = 0;

  let regulerBJ = 0,
    regulerModalMesin = 0,
    regulerModalAset = 0;
  let regulerBJ_Tunai = 0,
    regulerModalMesin_Tunai = 0,
    regulerModalAset_Tunai = 0;

  let kinerjaBJ = 0,
    kinerjaModalMesin = 0,
    kinerjaModalAset = 0;
  let kinerjaBJ_Tunai = 0,
    kinerjaModalMesin_Tunai = 0,
    kinerjaModalAset_Tunai = 0;

  let silpaKinerjaBJ = 0,
    silpaKinerjaModalMesin = 0,
    silpaKinerjaModalAset = 0;
  let silpaKinerjaBJ_Tunai = 0,
    silpaKinerjaModalMesin_Tunai = 0,
    silpaKinerjaModalAset_Tunai = 0;

  let grandTotalBJ = 0,
    grandTotalModalMesin = 0,
    grandTotalModalAset = 0;
  ...
  expenses.forEach((tx) => {
    ...
    if (SOURCE_IDS.LAINNYA.includes(id)) {
      if (isBarangJasa) {
        lainnyaBJ += amount;
        lainnyaBJ_Tunai += amountTunai;
      } else if (isModalMesinStrict) {
        lainnyaModalMesin += amount;
        lainnyaModalMesin_Tunai += amountTunai;
      } else if (isModalAsetStrict) {
        lainnyaModalAset += amount;
        lainnyaModalAset_Tunai += amountTunai;
      }
    } else if (SOURCE_IDS.REGULER.includes(id)) {
      if (isBarangJasa) {
        regulerBJ += amount;
        regulerBJ_Tunai += amountTunai;
      } else if (isModalMesinStrict) {
        regulerModalMesin += amount;
        regulerModalMesin_Tunai += amountTunai;
      } else if (isModalAsetStrict) {
        regulerModalAset += amount;
        regulerModalAset_Tunai += amountTunai;
      }
    } else if (SOURCE_IDS.KINERJA.includes(id)) {
      if (id === 35) {
        // SiLPA Kinerja
        if (isBarangJasa) {
          silpaKinerjaBJ += amount;
          silpaKinerjaBJ_Tunai += amountTunai;
        } else if (isModalMesinStrict) {
          silpaKinerjaModalMesin += amount;
          silpaKinerjaModalMesin_Tunai += amountTunai;
        } else if (isModalAsetStrict) {
          silpaKinerjaModalAset += amount;
          silpaKinerjaModalAset_Tunai += amountTunai;
        }
      } else {
        // Reguler Kinerja (2025)
        if (isBarangJasa) {
          kinerjaBJ += amount;
          kinerjaBJ_Tunai += amountTunai;
        } else if (isModalMesinStrict) {
          kinerjaModalMesin += amount;
          kinerjaModalMesin_Tunai += amountTunai;
        } else if (isModalAsetStrict) {
          kinerjaModalAset += amount;
          kinerjaModalAset_Tunai += amountTunai;
        }
      }
    } else {
  ...
  return {
    lainnya: {
      barangJasa: lainnyaBJ,
      modalMesin: lainnyaModalMesin,
      modalAset: lainnyaModalAset,
      tunai: lainnyaBJ_Tunai + lainnyaModalMesin_Tunai + lainnyaModalAset_Tunai,
    },
    reguler: {
      barangJasa: regulerBJ,
      modalMesin: regulerModalMesin,
      modalAset: regulerModalAset,
      tunai: regulerBJ_Tunai + regulerModalMesin_Tunai + regulerModalAset_Tunai,
    },
    silpaKinerja: {
      barangJasa: silpaKinerjaBJ,
      modalMesin: silpaKinerjaModalMesin,
      modalAset: silpaKinerjaModalAset,
      tunai: silpaKinerjaBJ_Tunai + silpaKinerjaModalMesin_Tunai + silpaKinerjaModalAset_Tunai,
    },
    kinerja: {
      barangJasa: kinerjaBJ,
      modalMesin: kinerjaModalMesin,
      modalAset: kinerjaModalAset,
...
function getReconciliationData(db, year) {
  ...
  for (let month = 1; month <= 12; month++) {
      ...
      month === 1
        ...
        : {
            bank:
              runningLainnya.bank +
              runningReguler.bank +
              runningSilpaKinerja.bank +
              runningKinerja.bank,
            tunai:
              runningLainnya.tunai +
              runningReguler.tunai +
              runningSilpaKinerja.tunai +
              runningKinerja.tunai,
            total:
              runningLainnya.bank +
              runningReguler.bank +
              runningSilpaKinerja.bank +
    ...
    const closingLainnyaTotal =
      runningLainnya.bank + runningLainnya.tunai + (income.danaLainnya || 0) - biayaLainnya;

    const biayaLainnyaTunai = expenses.lainnya?.tunai || 0;
    let closingLainnyaTunai =
      runningLainnya.tunai +
      (cashFlow.lainnya?.tarikTunai || 0) -
      (cashFlow.lainnya?.setorTunai || 0) -
      biayaLainnyaTunai;
    ...
    const biayaSilpaKinerja =
      (expenses.silpaKinerja?.barangJasa || 0) +
      (expenses.silpaKinerja?.modalMesin || 0) +
      (expenses.silpaKinerja?.modalAset || 0);

    const closingSilpaKinerjaTotal =
      runningSilpaKinerja.bank + runningSilpaKinerja.tunai - biayaSilpaKinerja;
    ...
    const closingKinerjaTotal =
      runningKinerja.bank + runningKinerja.tunai + incomeKinerja - biayaKinerja;

    let closingKinerjaTunai =
      runningKinerja.tunai +
      (cashFlow.kinerja?.tarikTunai || 0) -
      (cashFlow.kinerja?.setorTunai || 0);
    ...
    const closingRegulerTotal =
      runningReguler.bank + runningReguler.tunai + incomeReguler - biayaReguler;
    ...
    const closingTotal =
      closingLainnyaBank +
      closingLainnyaTunai +
      closingSilpaKinerjaBank +
      closingSilpaKinerjaTunai +
      closingKinerjaBank +
      closingKinerjaTunai +
      closingRegulerBank +
      closingRegulerTunai;
    ...
    const closing = {
      ...
      tunai:
        closingRegulerTunai + closingLainnyaTunai + closingSilpaKinerjaTunai + closingKinerjaTunai,
      total: closingTotal,
      details: {
  ...
  for (let q = 1; q <= 4; q++) {
    ...
    const sumExpenses = (path) =>
      quarterMonths.reduce((sum, m) => {
...
function getFundSourceDetail(db, year, fundSourceId) {
  ...
  const realizationQuery = `
        ...
        AND k.id_ref_bku NOT IN (5, 33, 10, 11)
        AND(
            COALESCE(k.id_anggaran, r.id_anggaran) IN(SELECT id_anggaran FROM RelevantBudgets)
          )
        GROUP BY month, k.kode_rekening
            `;

  const realizationRows = db.prepare(realizationQuery).all(yearStr, sourceConfig.pattern, yearStr);
  ...
  const monthlyBudgetQuery = `
        WITH RelevantBudgets AS(${budgetFilter})
        SELECT
          (p.id_periode - 80) as bulan,
          SUM(rp.jumlah) as total_anggaran
        ...
        WHERE r.id_anggaran IN(SELECT id_anggaran FROM RelevantBudgets)
          ...
          AND p.id_periode >= 81 AND p.id_periode <= 92
        GROUP BY bulan
            `;

  const monthlyBudgetRows = db.prepare(monthlyBudgetQuery).all(yearStr, sourceConfig.pattern);
  const monthlyBudgetMap = {};
  monthlyBudgetRows.forEach((row) => {
    monthlyBudgetMap[row.bulan] = row.total_anggaran;
  });
  ...
  let bjIdx = 1;
  colBarangJasa.forEach((c) => {
    c.displayIdx = bjIdx++;
  });

  let modalIdx = 1;
  colModalMesin.forEach((c) => {
    c.displayIdx = modalIdx++;
  });

  colModalLainnya.forEach((c) => {
    c.displayIdx = modalIdx++;
  });
  ...
  const calculateSummary = (monthsToSum, label) => {
    const summaryRow = {
      ...
      isSummary: true,
      values: {},
      total: monthsToSum.reduce((sum, mRow) => sum + (mRow.total || 0), 0),
    };
...
function getPajakDetail(db, year) {
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    if (m % 3 === 0)
      rows.push(
        createTaxSummary(monthlyData.slice((Math.ceil(m / 3) - 1) * 3, m), `TRIWULAN ${m / 3} `)
      );
...
function getBaAuditData(db, year) {
  ...
  result.expenses.silpaKinerja = db
    .prepare(expBase + '35) ORDER BY k.tanggal_transaksi')
    .all(yearStr);
